    ...
    public ResponseEntity<ApiResponse<Void>> followUser(
        ...
        if (existingOpt.isPresent()) {
            ...
            if (existing.getStatus() == ConnectionStatus.REJECTED) {
                // Allow re-follow after rejection by deleting the old rejected record
                connectionRepository.delete(existing);
            } else {
                return ResponseEntity.ok(ApiResponse.success("Already interacting (follow/pending)", null));
            }
        }

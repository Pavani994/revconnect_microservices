...
export class FeedPage implements OnInit {
    ...
    this.postService.getPersonalizedFeed(0, 10).subscribe({
      next: (response) => {
        if (response.success && response.data && response.data.content.length > 0) {
          this.populateFeed(response.data.content);
          this.isLoading = false;
          this.cdr.markForCheck();
        } else {
          this.fallbackToMyPosts();
        }
      },
      error: () => {
        this.fallbackToMyPosts();
  ...
  private fallbackToMyPosts() {
    this.postService.getMyPosts(0, 10).subscribe({
      next: (myRes) => {
        if (myRes.success && myRes.data) {
          this.populateFeed(myRes.data.content);
        }
        this.isLoading = false;
        this.cdr.markForCheck();
      },
  ...
  getRelativeTime(dateString: string): string {
    ...
    // Backend returns local time (no Z suffix) — parse as-is

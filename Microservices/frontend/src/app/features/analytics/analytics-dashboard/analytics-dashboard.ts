...
export class AnalyticsDashboard implements OnInit {
  ...
  loadData(): void {
    ...
    forkJoin({
      ...
      next: (results) => {
        if (results.overview?.success && results.overview.data) this.overview = results.overview.data;
        else this.overview = { totalViews: 0, totalLikes: 0, totalComments: 0, totalShares: 0, totalFollowers: 0, totalPosts: 0 };
        ...
        if (results.demographics?.data) this.audienceDemographics = results.demographics.data;
        else this.audienceDemographics = { personal: 0, creator: 0, business: 0, totalFollowers: 0, totalAudience: 0 };
